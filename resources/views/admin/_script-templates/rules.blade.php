    ...
    <div class="line rule-line mb10">
        <select class="form-control pull-left rule-a">
            @foreach($ruleGroups as $key => $row)
                <optgroup label="{{ trans('edutalk-custom-fields::rules.groups.' . $key) }}">
        ...
        <select class="form-control pull-left rule-type">
            <option value="==">{{ trans('edutalk-custom-fields::base.form.rules.is_equal_to') }}</option>
            <option value="!=">{{ trans('edutalk-custom-fields::base.form.rules.is_not_equal_to') }}</option>
        </select>
        ...
        <a class="location-add-rule-and location-add-rule btn yellow-lemon pull-left" href="#">
            {{ trans('edutalk-custom-fields::base.form.rules.and') }}
        </a>
...
<script type="text/x-custom-template" id="rules_line_group_template">
    <div class="line-group" data-text="{{ trans('edutalk-custom-fields::base.form.rules.or') }}"></div>
</script>